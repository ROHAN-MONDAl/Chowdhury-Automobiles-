...
// Prevent JSON breakage by buffering output
ob_start();
...
// ==========================================
// This block runs when you load the page/modal to fill the inputs
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        $id = isset($_GET['id']) ? intval($_GET['id']) : 0;

        if ($id <= 0) {
            throw new Exception("Invalid ID provided for fetching.");
        }

        // Join all tables to get the complete dataset
        $sql = "SELECT * FROM vehicle 
                ...
                LEFT JOIN vehicle_ot ON vehicle.id = vehicle_ot.vehicle_id 
                WHERE vehicle.id = $id LIMIT 1";
    ...
    // Stop script here for GET requests
    ob_end_flush();
    exit;
}

// ==========================================
// 3. UPDATE DATA (POST REQUEST - YOUR LOGIC)
// ==========================================
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // ⭐ NEW FEATURE: SYNC RELATED TABLES
        function syncRelatedTables($vid, $v_no, $name, $chassis, $engine) {
            global $conn;
            
            // 1. Update Seller Table
            $sqlSeller = "UPDATE vehicle_seller SET 
                          seller_vehicle_number = '$v_no',
                          seller_bike_name      = '$name',
                          seller_chassis_no     = '$chassis',
                          seller_engine_no      = '$engine'
                          WHERE vehicle_id = '$vid'";
            $conn->query($sqlSeller);

            // 2. Update Purchaser Table
            $sqlPurchaser = "UPDATE vehicle_purchaser SET 
                             purchaser_vehicle_no = '$v_no',
                             purchaser_bike_name  = '$name'
                             WHERE vehicle_id = '$vid'";
            $conn->query($sqlPurchaser);

            // 3. Update OT Table
            $sqlOT = "UPDATE vehicle_ot SET 
                      ot_vehicle_number = '$v_no'
                      WHERE vehicle_id = '$vid'";
            $conn->query($sqlOT);
        }
        ...
        function handleUpload($inputName, $existingFile) {
            ...
            if (in_array($ext, $allowed)) {
                ...
                $newName = time() . '_' . rand(100, 999) . '_' . $cleanName . '.' . $ext;
        ...
        // Fetch existing data strictly for file logic
        ...
        switch ($currentStep) {
            // ====================================================
            // STEP 1: VEHICLE MASTER (UPDATES SYNC HERE)
            // ====================================================
            case 1:
                ...
                // Clean variables for Sync
                $v_no = clean('vehicle_number');
                $name = clean('name');
                $chassis = clean('chassis_number');
                $engine = clean('engine_number');

                $sql = "UPDATE vehicle SET 
                    vehicle_type = '" . clean('vehicle_type') . "', name = '$name', 
                    vehicle_number = '$v_no', register_date = '" . clean('register_date') . "', 
                    owner_serial = '" . clean('owner_serial') . "', chassis_number = '$chassis', 
                    engine_number = '$engine', payment_type = '" . clean('payment_type') . "', 
                ...
                if ($conn->query($sql)) {
                    // ⭐ TRIGGER SYNC: Update other tables with new Master Data
                    syncRelatedTables($id, $v_no, $name, $chassis, $engine);
                } else {
                    throw new Exception("Step 1 Error: " . $conn->error);
                }
                break;
        ...
        if($currentStep === 1) {
             $response['message'] = 'Vehicle Master Updated & Synced!';
        }
    ...
    // Stop script here for POST requests
    ob_end_flush();
